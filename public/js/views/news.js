// /#/news — IT ニュース一覧 (履歴付き)。 v705 #297
//   過去 30 日分の記事を初出日付ごとにグループで表示。
//   各記事に GPT 要約 (日本語) があるなら併記。
...
async function load() {
  ...
  try {
    ...
    const items = d.items || [];
    if (!items.length) {
      root.innerHTML = '<div class="empty">まだ記事が蓄積されていません</div>';
      return;
    }
    // 日付ごとに group
            ...
            <div class="list-item" style="padding:6px 0; flex-direction:column; align-items:flex-start; gap:0; line-height:1.4">
              ...
              <div id="sum-${itemId}" style="width:100%; margin-top:2px">
                ${it.summary_jp
                  ...
                  : `<button class="btn news-load-sum" data-url="${escapeHtml(it.url)}" data-title="${escapeHtml(it.title)}" data-target="sum-${itemId}" style="font-size:11px; padding:2px 8px; color:#4a106d">🔄 要約を取得</button>`}
              </div>
            </div>`;
          }).join('')}
        </div>`;
    ...
    root.querySelectorAll('.news-load-sum').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          btn.disabled = false; btn.textContent = '🔄 要約を取得';
          toast('失敗: ' + e.message);
        }
      });
    });
