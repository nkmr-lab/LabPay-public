...
5. [サンプルクライアント](#サンプルクライアント) — Python / JS で動くひな型
...
LabPay は **同一オリジン専用** に絞られているので、ブラウザの別オリジン JS から直接叩くと CORS で蹴られます。対策:

- **静的ホスティングを LabPay と同じドメインに置く** (例: `pay.nkmr.io/hack/yourapp/`)
- **サーバサイドで proxy する** (Python / Node / etc から叩く分には CORS は無関係)
- **ブラウザ拡張** にしてホスト権限を取る
