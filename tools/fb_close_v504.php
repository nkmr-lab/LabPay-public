...
$BATCH = [
    122 => [
        'summary' => 'グループの 📅 スケジュール ヘッダに 「📥 Google Map」 ボタンを追加しました。 KML または GeoJSON を選ぶと、 既存スケジュールとタイトル一致 + 50m 以内のものはスキップして、 残りをストック (day_date=null) として一括追加します。 取り込んだあとは通常通り 「行きたい場所ストック」 から日付を設定する形になります。 KML/GeoJSON のパーサは places (食べある記) と共通の gmap_import.js に切り出したので、 同じ挙動と重複判定です。',
        'sns'     => '🗺 グループのスケジュールにも 「📥 Google Map」 取り込みボタン追加。 ストック (行きたい場所) に重複なしで一括追加できます #v504',
    ],
];
