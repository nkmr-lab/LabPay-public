...
export async function renderCg2() {
  ...
  app.innerHTML = `
    <div class="card page-header">
      <h2 style="margin:0">🎮 自作ゲーム v2 (cg2)</h2>
      <p class="hint" style="font-size:13px; margin-top:6px">
        p5.js で描画する准リアルタイム multiplayer の framework。
  ...
  try {
    ...
    if (!items.length) { root.innerHTML = '<div class="empty">まだゲームはありません</div>'; return; }
...
export async function renderCg2Kind({ params }) {
  ...
  app.innerHTML = `
    <div class="card">
      <a href="#/cg2" class="hint">← 自作ゲーム v2</a>
      <div id="cg2-kind-head" class="muted" style="margin-top:6px">読み込み中…</div>
    </div>
    <div class="card">
      <h3>卓一覧</h3>
      <button id="cg2-new" class="btn primary" style="margin-bottom:8px">＋ 新規卓</button>
...
export async function renderCg2Game({ params }) {
  ...
  app.innerHTML = `
    <div class="card">
      <a href="#/cg2/${escapeHtml(slug)}" class="hint">← 卓一覧</a>
  ...
  try {
    ...
    if (g.status === 'waiting') {
      ...
      const buttons = [];
      if (!am) buttons.push('<button id="cg2-join" class="btn primary">参加する</button>');
      if (isHost) {
        if (players.length < k.max_players) buttons.push('<button id="cg2-addai" class="btn">＋ AI を追加</button>');
      ...
      document.getElementById('cg2-cancel')?.addEventListener('click', async () => {
        if (!confirm('この卓をキャンセルしますか?')) return;
      ...
      // gameData に me_id を追加 (server 由来ではないので client で補う)
      const gameData = {
        ...g,
      ...
      // finished / cancelled
      ctrl.innerHTML = '<a class="btn" href="#/cg2/' + escapeHtml(slug) + '">卓一覧へ</a>';
    }
  } catch (e) {
...
function statusLabel(s) {
  return { waiting: '募集中', playing: 'プレイ中', finished: '終了', cancelled: 'キャンセル' }[s] || s;
}
function statusBg(s) {
