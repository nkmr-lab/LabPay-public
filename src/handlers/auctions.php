<?php
// /api/auctions — オークション MVP。 出品 + 入札 + lazy settle。 落札後の pt 移動は無し。
// Routes:
    ...
    if ($top) {
        ...
        try {
            Notifier::notify($pdo, $cfg, (int)$top['bidder_user_id'], 'auction',
                "🎉 落札しました!: 「{$a['title']}」 ({$top['amount']}pt)", 'auction', $id);
        } catch (Throwable $_) {}
        // 出品者に
        try {
            ...
            Notifier::notify($pdo, $cfg, (int)$a['seller_user_id'], 'auction',
                "🏷 落札確定: 「{$a['title']}」 → {$winName} さん ({$top['amount']}pt)", 'auction', $id);
        } catch (Throwable $_) {}
    } else {
...
function auctions_bid(PDO $pdo, array $cfg, int $id): void {
    ...
    db_tx($pdo, function () use ($pdo, $cfg, $u, $id, $amount) {
        ...
        if ($amount < (int)$a['min_price']) {
            throw new ApiException('bad_request', "最低 {$a['min_price']}pt から", 400);
        }
        ...
        if ($top && $amount <= (int)$top['amount']) {
            throw new ApiException('bad_request',
                "現在の最高入札 {$top['amount']}pt より高い額が必要", 400);
        }
        ...
        try {
            Notifier::notify($pdo, $cfg, (int)$a['seller_user_id'], 'auction',
                "💰 新しい入札: 「{$a['title']}」 {$amount}pt", 'auction', $id);
        } catch (Throwable $_) {}
        if ($top && (int)$top['bidder_user_id'] !== (int)$u['id']) {
            try {
                Notifier::notify($pdo, $cfg, (int)$top['bidder_user_id'], 'auction',
                    "⚠ 上を取られました: 「{$a['title']}」 → 新最高 {$amount}pt", 'auction', $id);
            } catch (Throwable $_) {}
        }
    });
