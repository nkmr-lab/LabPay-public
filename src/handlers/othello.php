...
function othello_initial_board(): array {
    $b = array_fill(0, 64, 0);
    // 中央 4 マスに初期配置 (黒: 3,4 と 4,3; 白: 3,3 と 4,4)
    $idx = fn($r, $c) => $r * 8 + $c;
...
function othello_create(PDO $pdo, int $uid): void {
    // v632 対象者指定 → 即起動: body.opponent_uid が渡ればその人を即着席させて mine_setup に
    $body = read_json_body();
    ...
    db_tx($pdo, function () use ($pdo, $uid, $oppUid, &$gid, &$oppName) {
        ...
        if ($oppUid) {
            if ($oppUid === $uid) throw new ApiException('bad_request', '自分は指定不可', 400);
            ...
            if (Ledger::balanceOfUser($pdo, $oppUid) < OTHELLO_FEE) {
                throw new ApiException('insufficient_balance', "{$oppName} さんのポイント不足で開始不可", 400);
            }
        }
    ...
    if ($oppUid && $gid) {
        try {
            ...
            notify_safely($pdo, $CFG, $oppUid, 'admin_notice',
                "💣 {$by} さんから地雷オセロ #{$gid} に招待されました (地雷配置へ)",
                'othello', $gid);
        } catch (Throwable $_) {}
    }
...
function othello_join(PDO $pdo, int $uid, int $gid): void {
    db_tx($pdo, function () use ($pdo, $uid, $gid) {
        ...
        if ((int)$g['creator_user_id'] === $uid) throw new ApiException('bad_request', '自分の卓には参加できません', 400);
        $bal = Ledger::balanceOfUser($pdo, $uid);
        if ($bal < OTHELLO_FEE) throw new ApiException('insufficient_balance', "ポイント不足 (要 " . OTHELLO_FEE . " pt)", 400);
...
function othello_set_mines(PDO $pdo, int $uid, int $gid): void {
    ...
    $clean = [];
    foreach ($cells as $c) {
        if (!is_string($c) || !preg_match('/^[0-7][0-7]$/', $c)) throw new ApiException('bad_request', '位置形式不正 (rrcc, 0-7)', 400);
        // 初期 4 マス (33,34,43,44) はダメ
        if (in_array($c, ['33','34','43','44'], true)) throw new ApiException('bad_request', '初期4マスには設置できません', 400);
        $clean[] = $c;
    }
    db_tx($pdo, function () use ($pdo, $uid, $gid, $clean) {
        ...
        if ($bothDone) {
            // v665 先手をランダムに (= creator / opponent のどちらか)
    ...
    // v625 AI 戦で開始直後 AI 番なら進める (通常は creator 始まりなので no-op)
...
function othello_move(PDO $pdo, int $uid, int $gid): void {
    ...
    db_tx($pdo, function () use ($pdo, $uid, $gid, $row, $col) {
        ...
        if (!$flips) throw new ApiException('bad_request', 'ここには置けません (相手の石を挟めない)', 400);
        ...
        if (!othello_has_move($board, $nextColor)) {
            ...
            if (othello_has_move($board, $myColor)) {
                $nextSide = $side; // 自分の番維持 (パスした側は後で UI から pass)
            } else {
                $finished = true;
            }
        }
        if ($finished) {
            ...
            if ($winner === 'draw' && !(int)$g['is_ai']) {
                $pot = (int)$g['pot_total'];
                $each = intdiv($pot, 2);
                Ledger::transfer($pdo, 1, (int)$g['creator_user_id'], $each, 'othello_refund', 'othello', $gid, "オセロ引分返金");
                Ledger::transfer($pdo, 1, (int)$g['opponent_user_id'], $each, 'othello_refund', 'othello', $gid, "オセロ引分返金");
            }
...
function othello_pass(PDO $pdo, int $uid, int $gid): void {
    db_tx($pdo, function () use ($pdo, $uid, $gid) {
        ...
        $color = $side === 'creator' ? 1 : 2;
        $board = json_decode($g['board_json'], true);
        if (othello_has_move($board, $color)) throw new ApiException('bad_request', '置けるマスがある場合はパスできません', 400);
        // 両者パス → 終局
        $oppSide = $side === 'creator' ? 'opponent' : 'creator';
...
function othello_cancel(PDO $pdo, int $uid, int $gid): void {
    db_tx($pdo, function () use ($pdo, $uid, $gid) {
        ...
        $g = $st->fetch(PDO::FETCH_ASSOC);
        if (!$g) throw new ApiException('not_found', 'not found', 404);
        if ($g['status'] !== 'waiting') throw new ApiException('bad_request', '対戦開始後はキャンセル不可', 400);
        if ((int)$g['creator_user_id'] !== $uid) throw new ApiException('forbidden', '起案者のみ', 403);
        Ledger::transfer($pdo, 1, $uid, OTHELLO_FEE, 'othello_refund', 'othello', $gid, "オセロキャンセル返金");
...
function othello_ai_new(PDO $pdo, int $uid): void {
    // v626 専用 bot を優先 (麻雀の東家と名前が混ざらないように)。
...
function othello_ai_pick_mine(): string {
    // 内側 4x4 のうち中央 4 マス (初期駒) を除いた 12 マスからランダム選択。
    $cands = [];
    for ($r = 2; $r <= 5; $r++) {
        for ($c = 2; $c <= 5; $c++) {
...
function othello_ai_drive(PDO $pdo, int $gid): void {
    for ($i = 0; $i < 8; $i++) {
        ...
        db_tx($pdo, function () use ($pdo, $gid, &$advanced) {
            ...
            // 地雷 (self or user の地雷を AI が踏むパターン)
            $cellKey = "{$r}{$c}";
            foreach (['creator','opponent'] as $owner) {
                $list = $mines[$owner] ?? [];
                if (in_array($cellKey, $list, true) && !in_array($cellKey, array_column($triggered, 'cell'), true)) {
