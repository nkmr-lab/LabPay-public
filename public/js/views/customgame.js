// v620 自作ゲーム の 汎用ディスパッチャ。 ユーザ登録した kind を /#/cg/:kind で 受ける。
//   - /api/custom-games/list から kind の メタ (js_module_url) を 取得
//   - ES module を 動的 import して renderList / renderDetail を 呼び出す
//   - kind の JS module 側 は 任意の export 名で OK (例: renderTicTacToe) だが、
//     v620 から 推奨は renderList(ctx) / renderDetail(ctx)
//   - フォールバック として render{KindCamel} / render{KindCamel}Detail も 探す
...
async function loadKindModule(kind) {
  ...
  const p = (async () => {
    ...
    const url = meta.has_js_source
      ...
      : meta.js_module_url;
    // 動的 import (相対 URL でも 絶対 URL でも OK)
...
function pickRender(mod, kind, suffix) {
  // renderList / renderDetail を 優先、 なければ render{KindCamel}{Suffix} を 試す
...
export async function renderCustomGameList(ctx) {
  ...
  try {
    ...
    if (!fn) return renderError(`${kind} に renderList が ありません`);
    await fn(ctx);
  } catch (e) {
    renderError(`ゲーム読み込み 失敗: ${e?.message || e}`);
  }
}
...
export async function renderCustomGameDetail(ctx) {
  ...
  try {
    ...
    if (!fn) return renderError(`${kind} に renderDetail が ありません`);
    await fn(ctx);
  } catch (e) {
    renderError(`ゲーム読み込み 失敗: ${e?.message || e}`);
  }
