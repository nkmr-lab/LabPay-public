...
$BATCH = [
    111 => [
        'summary' => 'ホームのウィジェットの 「読み込み中…」 を、 軽い shimmer アニメーション付きの skeleton bars に置き換えました。 枠組みが即出てから 「中身を読んでる」 ことが視覚的に分かるようになり、 何が表示されようとしているのか / どこを待っているのかが見えるようになります。 順次表示の挙動 (各カードが完了したものから差し替わる) はそのまま維持しています。',
        'sns'     => '🦴 ホームのウィジェットを skeleton shimmer 風に。 枠組みを先に出してロード中であることを視覚化 #v501',
    ],
    115 => [
        'summary' => 'ホームのロード時間とボトルネックを可視化しました。 (1) 各セクション (balance / presence / freshlistings ...) の所要 ms と全体時間を console.groupCollapsed にダンプ — DevTools で誰でも見られます。 (2) admin 限定で画面右下にピル状の totalMs バッジが出ます。 タップでセクション別 ms の一覧に展開、 ホームから離れると自動で消えます。 これでどこが重いかひと目で分かるので、 次の高速化対象を絞れます。',
        'sns'     => '📊 ホームのロード時間を計測してダンプ + admin だけ右下にピル表示。 重い箇所がひと目で分かるように #v501',
    ],
];
