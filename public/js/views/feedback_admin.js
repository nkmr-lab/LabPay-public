...
export async function renderFeedbackAdmin() {
  ...
  document.getElementById('app').innerHTML = `
    <div class="card" id="fb-claude-dash">
      <h3 style="margin:0 0 6px">🤖 Claude 巡回状況</h3>
  ...
  `;
  // navigate 中の race で DOM が消えてる事があるので防御的に。
...
async function renderClaudeDashboard() {
  ...
  try {
    ...
    const pollWarn = lastPoll && (Date.now() - lastPoll.getTime() > 90 * 60 * 1000)
      ? ` <span class="tag" style="background:#ffe9c2; color:#9a4400">⚠ 1.5 時間以上前</span>`
      : '';
    const stuckCount = items.filter(it => it.age_seconds > 3600).length;
    const stuckTag = stuckCount > 0
      ? ` <span class="tag" style="background:#fee2e2; color:#b91c1c">⏸ ${stuckCount} 件滞留</span>`
      : '';
    const lastDoneLine = lastDone
      ? `<div>直近完了: #${lastDone.id} — ${escapeHtml(lastDone.claude_summary || '(無題)')} <span class="muted">${escapeHtml(lastDone.claude_finished_at || '')}</span></div>`
      : '<div class="muted">直近完了: なし</div>';
        ...
        ${items.map(it => {
          ...
          <div class="list-item">
            ...
            <div class="row" style="gap:4px; flex-direction:column">
              <button class="btn" data-dash-block-fb="${it.id}" style="font-size:11px; padding:2px 6px" title="一旦退避 (none に戻す)">退避</button>
              <button class="btn danger" data-dash-blocked-fb="${it.id}" style="font-size:11px; padding:2px 6px" title="Claude 対象から外す (blocked)">対象外</button>
            </div>
          </div>`;
        }).join('')}
      </div>` : '<div class="muted" style="margin-top:6px">approved / working なし</div>';
    root.innerHTML = `
      <div>最終巡回: <b>${escapeHtml(lastPoll ? lastPoll.toLocaleString() : '—')}</b> (${pollAgo})${pollWarn}${stuckTag}</div>
      ${lastDoneLine}
      ${itemsHtml}
    `;
    ...
    root.querySelectorAll('[data-dash-blocked-fb]').forEach(b => {
      b.addEventListener('click', async () => {
        if (!confirm('Claude 対象から外しますか? (blocked)')) return;
        try {
    ...
    root.querySelectorAll('[data-claude-fb]').forEach(b => {
      b.addEventListener('click', async () => {
        ...
        try {
          ...
          toast(next === 'approved' ? '🤖 Claude に任せました' : '取り消しました');
          await loadList();
        } catch (e) { toast('失敗: ' + e.message); }
      });
    });
...
function row(f) {
  ...
  // v407 Claude 自動対応ステータス badge + トグル
  ...
  // v703 #292 すべて表示で長文が横にはみ出して横スクロール発生 → 各テキスト
  //   コンテナに overflow-wrap:anywhere を強制して改行させる。
