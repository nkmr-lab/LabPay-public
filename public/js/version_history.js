...
export const VERSION_HISTORY = [
  ...
  { v: 'v711', d: '2026-06-20', s: '🔗 点呼 / 待ち合わせ / 投票 の 詳細 に も 「🔗 URL」 コピー ボタン を 追加 + タスク 一覧 で 「🎯 募集」 「👤 割り当て」 「🙏 依頼」 タグ で 区別、 「結果 待ち」 タグ を シンプル化 (#303 #304 #305)。 タスク 作成 ボタン も リクエスト → 依頼 に 名称 変更 で 統一' },
  { v: 'v710', d: '2026-06-20', s: '🗳 投票 / アンケート の 集計 に 「誰 が どれ に 投票 したか」 を 追加 + 「現状 何人中何人」 を 明示 (#302)。 各 option の 下 に 投票者 chip (アバター + 名前) を 並べる。 % と 棒 グラフ は 「回答済 M 人」 を 分母 に した もの に 変更 (投票 予定 を 含まない 現状 ベース)、 票数 表示 も 「N 票 / 回答済 M 人 (P%)」 形式 に' },
  { v: 'v709', d: '2026-06-20', s: '🔗 タスク / 募集 の 詳細 に 「🔗 URL」 ボタン (= 絶対 URL を クリップボード に コピー) を 追加 (#301)。 既存 の 「💬 共有 (らぼったー)」 に 加えて、 Slack や DM 等 LabPay 外 に そのまま 貼れる 形 で 共有 できる ように。 HTTPS 外 / 権限 拒否 時 は textarea fallback で 救う' },
  { v: 'v708', d: '2026-06-20', s: '👥 募集 の 「人数」 を 発起人 を 含まない 既定 に 変更 + 「自分 も 含める」 チェック ボックス を 追加 (#300)。 新規 募集 form / 編集 modal の 既定 は 自分 別 (= capacity_excludes_creator=1)、 検査 は 実 上限 = capacity + 1。 一覧 / 詳細 表示 は 「募集 N 人 (発起人 別)」 か 「参加 X / 上限 Y」 を flag で 切替。 既存 募集 は 旧 互換 (flag=0)' },
  { v: 'v707', d: '2026-06-20', s: '💬 タスク / リクエスト / 募集 の 詳細 に 「💬 共有」 ボタン を 追加 (#299)。 タップ で らぼったー 共有 modal が 開き、 タイトル と 詳細 へ の hash URL (#/tasks/ID 又は #/invitations/ID) が プリセット される。 誰 でも 押せる、 起案者 でなく て も 仲間 を 集める の に 使える' },
