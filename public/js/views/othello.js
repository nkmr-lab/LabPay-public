...
async function paintBoard(gid) {
    ...
    if (d.me_side === 'creator') {
      actionArea = `
        <div class="card">
          ...
          <button id="ot-cancel" class="btn" style="margin-top:8px">キャンセル (1pt 返金)</button>
        </div>`;
    } else {
      actionArea = `
        <div class="card">
          <div class="hint">対戦相手として 参加 しますか? (1pt)</div>
          <button id="ot-join" class="btn primary" style="margin-top:8px">参加する (1pt)</button>
        </div>`;
    }
  ...
  document.getElementById('ot-cancel')?.addEventListener('click', async () => {
    if (!confirm('キャンセルしますか? (1pt 返金)')) return;
