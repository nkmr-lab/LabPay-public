...
5. experiments: 論文 が 行った 実験 / 調査 を 「実験 N: 何 を 測った / 何 を 操作 した /
   被験者 N=◯◯ / 期間 / 統制 群 等」 形式 で 列挙。 自前 の 実験 が ない 理論 論文 でも、
   本文 で 取り上げた 重要 な 先行 研究 の 実証 結果 (例: 「Smith ら の 実験 で N=120 が ◯◯」)
   が ある なら それ を 同 形式 で 列挙 して 「(引用) Smith et al. の 実験」 と 明示 する。
   完全 に 該当 ゼロ の 純 理論 論文 のみ 空 配列。
6. results_summary: 上 の 実験 で 「何 が 分かった か」 を 数値 / 効果量 / p値 / 信頼区間 込み
   で 整理 (引用 研究 の 結果 含 む)。 該当 なし なら 空 配列。
...

{
  ...
  "experiments": [
    "実験 N: 「○○ を 測定 した」 (操作 変数 / 統制 変数 / 被験者 数 N=◯◯ / 期間 / 統制 群 等 を 1 文 で 整理)"
  ],
  "results_summary": [
    "結果 N: 「○○ 条件 で × より △% 高い、 p<.01、 効果量 d=0.5」 (実験 N に 対応 する 結果)"
  ],
