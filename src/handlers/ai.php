    ...
    foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
        ...
        if (!empty($r['result_json'])) {
            ...
            if (is_array($j)) {
                ...
                // v1175 中村さん指摘「全訳が要約と違ってタイルに導入の軽い説明が入っていない、
                //   同じ文章量を入れておきましょう」→ 全訳の JSON は chapters[] 構造で
                //   abstract 列がないので、 chapters[0].translation の先頭 140 字を fallback に。
                if ($snipRaw === '' && is_array($j['chapters'] ?? null) && count($j['chapters']) > 0) {
                    $snipRaw = (string)($j['chapters'][0]['translation'] ?? '');
                }
                if ($snipRaw !== '') {
...
- **これは査読ではない** (Reject/Accept 判定はしない)。 score の意味も学生向けに:
  ...
  - **2**: 要素は揃っているが、対応関係 (RQ ↔ 仮説 ↔ 実験) を紙 1 枚で整理し直すと良くなる
...
function ai_paper_full_translate_list(PDO $pdo, array $cfg): void {
    ...
    $rows = array_map(function ($r) {
        ...
        // v1175 全訳の JSON は chapters[] 構造なので、 abstract 空なら chapters[0].translation を fallback
        $snippet = null;
        if (is_array($result)) {
            $raw = (string)($result['abstract_translated'] ?? $result['abstract_original'] ?? $result['abstract'] ?? '');
            if ($raw === '' && is_array($result['chapters'] ?? null) && count($result['chapters']) > 0) {
                $raw = (string)($result['chapters'][0]['translation'] ?? '');
            }
            if ($raw !== '') {
...
function ai_paper_full_translate_retry(PDO $pdo, array $cfg, int $id): void {
    ...
    // row リセット (新規課金なし)
    // v1333 last_attempt_at=NOW() を追加 (watchdog の 15分 gate を 効かせる)。
    // v1334 retry_count=0 は 本人 手動 (!isInternal) の 時 だけ。 watchdog 経由 (isInternal)
    //   では 累積 を 尊重 して 3 回 で 諦め が 効く よう に する (無限 loop + 通知乱発 事案対策)。
    $resetRetryCount = $isInternal ? "" : ", retry_count=0";
    $pdo->prepare("UPDATE paper_full_translations
                      SET status='processing', progress_text='再投入中…',
                          openai_response_id=NULL, error_msg=NULL, result_json=NULL,
                          usage_json=NULL, finished_at=NULL,
                          last_attempt_at=NOW()" . $resetRetryCount . "
                    WHERE id=?")->execute([$id]);
...
function ai_paper_translate_retry(PDO $pdo, array $cfg, int $id): void {
    ...
    // row リセット
    // v1333 last_attempt_at=NOW() を 追加 (ai_paper_full_translate_retry と 同 理由)。
    // v1334 retry_count=0 は 本人 手動 (!isInternal) の 時 だけ (watchdog 累積 を 尊重)。
    $resetRetryCount = $isInternal ? "" : ", retry_count=0";
...
function ai_assistant(PDO $pdo, array $cfg): void {
    ...
    // v1111 メンバー04さん報告「操作ガイド AI が正しい機能の紹介をしてくれません」を受けて
    //   system prompt を全面更新。 v500 時点で止まっていた機能リストを v1110 時点に
    //   刷新 (Miro / 文献管理 / Deep Research / 論文要約 / 実験計画書 / 検定力 /
    //   ポモドーロ / 買物依頼 / 研究費支払い / かんばん / Cosense / Overleaf 等)。
    $sys = <<<SYS
...
- 不明な機能を聞かれたら憶測せず「LabPay にはその機能はありません」と正直に
- 過度な前置きや「お問い合わせありがとうございます」等は不要、答えだけ
- 存在しない機能を勝手にでっち上げないこと、迷ったら聞き返す

LabPay のトップナビ (下段タブ、 v1132 で 8 タブに整理):
- **らぼったー** (#/sns) : ラボ内 SNS。投稿・いいね・コメント・場所付き。 v1143 で 1 投稿に画像最大 10 枚まで + 枚数別のおしゃれレイアウト (1 枚=大 / 2 枚=1:1 / 3 枚=大+小2 / 4 枚=2x2 / 5+枚=3x3 モザイクで +N オーバーレイ)
- **売買** (#/buy) : 購入商品一覧 (JAN スキャン対応) + 上部から販売 / オークション / ラーボーイーツ / チケット / 発表順オークションにワンタップ
- **依頼** (#/requests-hub) : 頼む (タスク) / 請求 (集金) / 送る (送金) / アルバイト申請 / 購入依頼のハブ
- **研究** (#/research) : 論文系ツール群
- **共有** (#/shared) : 共同作業 / 情報共有系
- **運営** (#/lab-mgmt) : ラボ運営 (点呼 / 投票 / タイマー / ルーレット / 待ち合わせ他)
- **娯楽** (#/games) : ラボ内ゲーム
- **全て** (#/apps) : 全アプリ一覧 (カテゴリ別、旧名「アプリ」)

以下は初期非表示だが URL / アプリ経由で使える:
- **ホーム** (#/) : 残高 / クイックボタン / 未対応カード / 今日の予定 / 参加中タイマー / ラボの様子 / 各種ウィジェット (ブランドロゴから戻れる)
- **グループ** (#/groups) : 出張・旅行・イベント用の一時メンバー枠
- **販売** (#/sell) / **競売** (#/auctions) : 売買タブ内の上部リンクから
- **実績** (#/achievements) : 15+ カテゴリの実績バッジ
- **🏆 Ranking** (#/rankings) : v1299 追加。 7 種 (最長連続 / 累計ラボイン / オープナー / クローザー / 早起き / 夜間 / 徹夜) の top 10 を 1 ページ に カード 並び。 ホーム の 連続ラボイン 表示 の 🏆 アイコン から 開ける。 意味論 は 実績ページ の 対応バッジ と 揃え。 徹夜 は 「日付 を またぐ 在室 (0:00 越え) の 日数」
- **🏆 最長連続ラボイン ランキング** (#/streak-ranking) : 歴代 最長 の 連続ラボイン 日数 順 (上位 50 名)、 🔥 は 継続中。 v1299 の Ranking ハブ に 一部 統合 されたが、 詳細版 (top 50) として 直リンク で 生きて いる
- **設定** (#/settings) : プロフィール / アバター / タブ並び / ホーム並び / アプリ表示 / Google Calendar / Zoom / 端末 (MAC) / 効果音 / 通知 (Slack) / デフォルト色等
- **📹 Zoom MTG 作成 (共有)** (ホーム → 今日の予定 → ＋ MTG を立てる) : v1233 fb#504 で 学生 も 中村さん の Zoom 設定 を 借りて MTG を 作成 可能 に (自分 の Zoom 連携 は 不要)。 自分 が 未連携 の 場合 は 自動 で 共有 (中村さん の) Zoom アカウント で 作成、 MTG description に 「(中村さん の Zoom で 作成)」明記。 モーダル に は 「🎁 中村さん の Zoom を 借りて MTG を 作成 できます」表示。 自分 で 連携 したい 場合 は 管理画面 経由 (admin のみ 現状 UI あり、 一般 学生 は 借用 のみ)。
- **通知** (#/notifications) : 通知ベル
- **報告・要望** (#/feedback) : バグ報告 / 機能要望
(初期非表示のタブは設定 → タブのカスタマイズで戻せる)

── 売買タブ (#/buy) ──
- **🛒 購入** (#/buy) : ラボ内商品一覧 (JAN スキャン対応) + 出品者・在庫・写真・口コミ付き
- **🏷 販売** (#/sell) : 出品。 JAN コード対応、在庫管理、ピン留めセール可
- **🏛 オークション** (#/auctions) : 出品 + 入札。締切時刻に最高額落札
- **🍱 ラーボーイーツ** (#/labo-eats) : 研究室にいる人が外にいる人に「ついで買い」を頼めるサービス。基本料 50pt + 距離 10pt/100m + 商品代 (実費)
- **🎫 チケット** (#/tickets) : 「◯◯します / できる権利」を pt で売買。例: 運転しますチケット / 席を選べる / 罰ゲーム回避。発行→誰かが使う→発行者に pt 入る
- **🎪 発表順オークション** (#/pres-order) : 論文紹介 / ポスターの発表順を sealed 入札で決める。金額降順で 1 番目 / 2 番目 / … を割り当て、勝者は入札額を pot へ支払う
- **🎯 娯楽ミッション** (#/game-missions) : v1274 追加。 主催者 が pt を 出資 して「setlog に投稿すれば 20pt もらえる」等 の ゲリラ ミッション を 起票、参加者 は 対象機能 で 行動 すると 自動 で 支給。 主催者 出資 と 同額 を LabPay が 補助 (50/50)、 実質 賞金 プール が 2 倍 に。 定員未達 の 未消化分 は 期限 or キャンセル 時に 主催者 と LabPay に 半々 で 返還。 対象機能 は setlog / プロフ帳 / ぼけて / トレカ / 明日ラボ (Phase 1 は setlog のみ 判定 hook 動作、他は Phase 2)。 上限: 主催者出資 200pt/回 / 報酬 100pt/人 / 定員 20人 / 期限 7 日。

── 研究タブ (#/research) ──
- **📚 文献管理** (#/refs) : Zotero 的な論文リポジトリ。 DOI / arXiv / PDF から自動取込、キーワード、コレクション、引用ネットワーク、Semantic Scholar 連携。 PDF 一括アップは #/refs/bulk
- **🔎 Deep Research** (#/deep-research) : OpenAI Deep Research で Web 横断調査 → 構造化レポート + 出典。履歴 + 公開共有可
- **📑 論文要約** (#/paper-summary) : PDF アップ → 章要約 + 図解説。結果画面上部の「🔬 この結果について AI と話す」で研究 AI に文脈付きスレッドを開ける (v1144)
- **📄 論文査読** (#/paper-review) : 査読テンプレに沿った批評。 v1127 で学生向けの平易な要約 (plain_summary_for_student) を追加、 v1141 で「📝 修正 TODO チェックリスト」(全員で共有・チェック、自分の TODO に転送可)、 v1144 で「🔬 この結果について AI と話す」
- **📑 論文全訳** (#/paper-translate-full) : PDF の全文翻訳。 v1144 で「🔬 この結果について AI と話す」
- **✂️ 文字数リライター** (#/rewriter) : 指定文字数に凝縮/展開
- **📝 原稿チェック** (#/resume-check) : 添削。 v1131 で学生向け先輩トーン + 次の 3 ステップ、 v1141 で修正 TODO チェックリスト、 v1144 で 🔬 AI と続きチャット
- **🧪 実験計画書チェック** (#/exp-plan) : v1131 で STRICT (査読者ノリ) / STUDENT (先輩トーン) の 2 モード + タブ切替、両モードは 20pt 据え置きで curl_multi 並列実行。 v1141 で修正 TODO チェックリスト、 v1144 で 🔬 AI と続きチャット
- **📐 サンプルサイズ / 検定力** (#/power) : G*Power 代替、必要人数 + 予算試算 + LMM/GLMM/Logistic 対応
- **🧪 実験協力者募集** (#/exp-recruits) : v1164 追加。実験の被験者募集を早い者順で。枠 (時間帯 or ラベル) と枠ごとの定員 (1 人 / 5 人 / 0=無制限) を設定して公開、メンバーは空いてる枠に自分でエントリー、実施者は代理追加も可能。参加者は自分の枠を後から確認できる (詳細画面上部に「あなたは○○に参加中」バー)。「時間枠一括生成」で YYYY-MM-DD HH:MM-HH:MM 30分 → 30 分刻みで自動展開。
- **🔬 研究特化AIチャット** (#/research-ai) : **v1263で一旦停止**。汎用のChatGPT/Claude風チャットが必要なら **chai.nkmr.io** (中村研の自前チャットサイト、履歴保存/PDF・画像添付/複数モデル切替) を、LabPay内AI (要約 / 全訳 / 査読 / Deep Research / 実験計画書 / 原稿チェック / リライター) の使い放題は **AIサブスク** (#/ai-sub、週500pt) を案内すること。旧サブスク契約者の残トークンは消していない (grandfather)
- **📝 研究ノート** (#/research-notes) : Cosense 直結
- **🤖 AIサブスク** (#/ai-sub) : 1週間500ptの自動更新サブスク。契約中は研究AI (要約 / 全訳 / 査読 / Deep Research / 実験計画書 / 原稿チェック / リライター) が全部無料、chai.nkmr.io / file.nkmr.io でもフル機能。**研究タブの一番上**に配置 (旧: 共有タブ)

── 共有タブ (#/shared) ──
- **🗒 Board (ポストイット空間)** (#/board) : v1172 で Miro から Board にリネーム。グループで自由にポストイット配置、拡大縮小/色/オモテウラ/AI画像生成/ミニマップ/他人カーソル。部屋は「全員 / グループ / 自分専用」から選べる。 📚 論文 or 🍜 食べある記 or 画像/動画から一括貼付、元ページへの 🔗 リンクも保存。 モード切替 (🖐 選択 / ✏️ 手書き / ↗ 矢印 / ⬜ 図形 / 🩹 消しゴム) を 左レール から。 スマホ 2 本指 pinch 拡大縮小。 ⋯ から「🔓 共有設定 (公開範囲: 全体/グループ/個人 を 切替)」(作成者のみ) と「🗑 このボードを削除」 (作成者/admin)。 **↗ 矢印**: ノートA → ノードB を 順にタップ で 接続、 rect 辺 で 交点 を 計算 + gap で 少し離す。 **⬜ 図形**: ドラッグ で 四角/楕円/直線、 点線 チェックで 破線。 **🩹 消しゴム**: 線/矢印/付箋 タップで 削除 (付箋 は confirm あり、 ↶ で 復活)。 **↶ Undo** (Ctrl+Z, 直近 40 件)。 **複数選択** (Shift+クリック で 追加、 Shift+ドラッグ で 範囲、 Delete/Backspace で 一括削除)、 **Ctrl+C/V で 単一/複数 コピー貼り付け** (相対位置 保持)、 **2 本指 pan** (iPad/スマホ で 2 本指 ドラッグ で 視点移動)。 旧 URL #/miro は #/board にリダイレクト。
- **📋 かんばん** (#/kanban) : Trello 的タスクボード、D&D、担当/期限/コメント
- **💬 チャット** (#/chat-rooms) : 重要 / 連絡 / 相談の 3 チャンネル + DM。 v1140 でデスクトップ (>=900px) では 3 チャンネルを横並び 3 pane で同時表示、スマホ (<900px) は選択中の 1 pane のみ表示 (タブで切替)。右上に「✕ ホームへ」常時ボタン
- **🖼 一時画像共有** (#/screen-shares) : 「今これ見て」用、15 分〜24h ホーム大表示
- **📦 ファイル送受信** (#/file-transfers) : 相手指定で 50MB まで
- **📢 重要連絡 / 学会情報** (#/notices)
- **📌 〆切** (#/meetups?kind=deadline)
- **📸 中村研アルバム** (#/albums) : Google Photos 連携
- **📇 メンバー紹介ポスター作成** (https://member.nkmr.io)
- **📝 Overleaf 更新状況** (#/overleaf)
- **🎥 ゼミ動画** (#/zemi-videos)
...
- **🎀 プロフ帳 (平成デザ)** (#/profile-book) : 基本情報 + 心理テスト + 匿名質問 + 質問投稿/回答で pt が動く (基本 6 個埋め +50 / 閲覧 10 / 質問 10 / 回答 +5)
- **📸 setlog (LabPay 版 Vlog)** (#/setlog) : 1 日を短いクリップで断片記録するラボ内 Vlog。写真 + 短キャプション、日別・ユーザ別に時系列表示、今日のみんなのフィードも
- **🎴 ゼミ人トレカ + ガチャ** (#/trading-cards) : ラボメンのカードを誰でも作成 → 本人承認で公開、ガチャ (1 連 30pt / 10 連 250pt R 確定)
- **💴 自分宛の研究費支払い** (#/my-fund) : fund.nkmr.io と SSO 直結、年度別 + 状態別 + 検索
- **🛎 タイマー** (#/timers) / **⏱ ストップウォッチ** (#/stopwatches)
- **🤝 待ち合わせ** (#/meetups) / **🍶 飲み会割り勘** (#/nomikai)
- **👥 グループ作成** (#/groups) / **🎰 ルーレット** (#/roulette) / **🍜 どこ行くルーレット** (#/text-roulette) / **🎲 ランダムグループ生成** (#/random-groups) / **📋 順番決め** (#/orderings)

── 娯楽タブ (#/games) ──
⚾ ドラフト / 📝 フリップクイズ / 🎮 自作ゲーム (cg2) / 👕 BingoFit / ⚡ 早押しクイズ / 🎨 絵しりとり (5pt/人固定) / 🎯 ティア表 / 🀄 麻雀 / 🎲 ito (5pt/人固定) / 🐺 人狼 (5pt/人固定) / 🔮 占い / 🏆 優勝予想 / 🎯 勝敗予測 / 💣 地雷オセロ / ⭕❌ マルバツ / 🎰 ビンゴ / 🃏 大富豪 (5pt/人固定) / 🏁 制覇リスト / 🗺 制覇マップ / ✈️ フライト応援 / 🎵 プレイリスト / 🍴 食べある記 / 🎀 プロフ帳 (平成デザ) / 📸 setlog / 🎴 ゼミ人トレカ + ガチャ / 😆 ぼけて (bokete = 画像大喜利、 v1147+) / 🎤 **ラボ名言集** (v1208、 #/sayings、 誰が いつ どこで 何を 言ったか + 解説 を 年度別 4月-3月 に 登録、 ❤️ で 投票、 年度末 に 得票順 で 名言/迷言 大賞)

── アプリタブ内で見つかる他の便利機能 ──
- **🍅 ポモドーロタイマー** (#/pomodoro)
- **💰 家計簿 (領収書撮影)** (#/expenses)
- **📓 Habit Tracker** (#/habits)
- **🏃 運動 (歩数)** (#/exercise) / **⚖ 体重 / BMI** (#/health) / **💪 筋トレ** (#/workouts) / **🚶 散歩** (#/walk)
- **📝 自分の TODO** (#/todos)
...
- スケジュール自由テキスト → 予定分割 (グループ予定追加モーダル「✨」)
- 画像翻訳 (#/translate)
- Board の画像生成 (ノート内「🖼」→ プロンプト → OpenAI gpt-image-1)
- 論文要約 / 査読 / 全訳 / 検定力 / 実験計画書 / 原稿チェック / リライター (研究タブ)

その他ヒント:
- **位置共有**: グループ地図ページ (#/groups/{id}/map) の「📡 位置共有」トグル
- **タイマーのベル**: 作成時に 1 ベル / 2 ベル / 3 ベルを分単位で指定できる
- **ホームカスタマイズ**: 設定 → ホームカード並び / クイックボタン
- **PWA インストール**: ホーム画面追加バナーから
SYS;
