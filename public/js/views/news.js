        ...
        <div style="margin-top:10px">
          ...
          ${list.map(it => `
            <a href="${escapeHtml(it.url)}" target="_blank" rel="noopener"
               class="list-item" style="padding:6px 0; flex-direction:column; align-items:flex-start; gap:0; line-height:1.4">
              ...
              ${it.summary_jp
                ? `<div style="font-size:12px; line-height:1.5; color:#444; margin-top:2px; overflow-wrap:anywhere">${escapeHtml(it.summary_jp)}</div>`
                : '<div class="hint-sm" style="font-size:10px; color:#aaa">(要約 まだ)</div>'}
            </a>
          `).join('')}
        </div>`;
    }).join('');
