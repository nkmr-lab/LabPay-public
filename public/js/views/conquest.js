// /#/conquest — ユーザ が 自由 に 「制覇 リスト」 を 作って、 アイテム を 追加 + 自分 が
//   達成 した もの を チェック していく アプリ。 v860 #445。
//   例: 「中野区 の パン屋」 「都内 の ラーメン屋 一蘭」 「47 都道府県 県庁所在地」 …
...
export async function renderConquest() {
  ...
  app.innerHTML = `
    <div class="card page-header">
      <div class="row center" style="gap:6px; flex-wrap:wrap">
        <h2 style="margin:0">🏁 制覇 リスト</h2>
        <span style="flex:1"></span>
        <a class="btn primary" href="#/conquest/new">＋ 新しい リスト</a>
      </div>
      <div class="hint-sm" style="margin-top:4px">
        街 の パン屋、 通っ た 学会、 行きたい 温泉 など、 自分 だけ の 制覇 リスト を 作って チェック していけます。
      </div>
    </div>
  ...
  try {
    ...
    if (!items.length) {
      document.getElementById('cq-list').innerHTML = `
        <div class="card center muted">
          まだ リスト が ありません。 「＋ 新しい リスト」 から 作って みよう。
        </div>`;
      return;
    }
    ...
    document.getElementById('cq-list').innerHTML = `<div class="card muted">読み込み 失敗: ${escapeHtml(e.message)}</div>`;
  }
}
...
export async function renderConquestNew() {
  ...
  app.innerHTML = `
    <div class="card page-header">
      ...
      <h2 style="margin:6px 0 0">＋ 新しい 制覇 リスト</h2>
    </div>
    <div class="card">
      <label style="font-size:13px">タイトル <span class="muted">(例: 中野区 の パン屋)</span></label>
      ...
      <select id="cq-vis" style="padding:8px; margin-top:4px">
        <option value="public">🌐 公開 (誰でも 見られる、 みんな で 追加 OK)</option>
...
export async function renderConquestDetail({ id }) {
  ...
  app.innerHTML = `
    ...
    ${canAddItem ? `
      <div class="card">
        <div class="bold" style="font-size:13px; margin-bottom:6px">＋ アイテム を 追加</div>
        <div class="row" style="gap:6px; flex-wrap:wrap">
          <input id="cq-item-name" type="text" maxlength="160" placeholder="お店 名 / 対象 名" style="flex:1; min-width:160px; padding:8px">
          <input id="cq-item-note" type="text" maxlength="400" placeholder="メモ (住所 等、 任意)" style="flex:1; min-width:160px; padding:8px">
    ...
    <div class="card">
      ...
      <div id="cq-items">
        ${d.items.length === 0
          ? '<div class="muted">まだ アイテム が ありません。 上 から 追加 してください。</div>'
          : d.items.map(it => renderItemRow(it, d)).join('')}
      </div>
    </div>`;
  if (d.is_mine) {
    document.getElementById('cq-del')?.addEventListener('click', async () => {
      if (!confirm('この リスト を 削除 します。 いいですか?')) return;
      try { await del('/api/conquest/lists/' + id); toast('削除 しました'); navigate('#/conquest'); }
      catch (e) { toast('失敗: ' + e.message); }
    });
  }
  if (canAddItem) {
    document.getElementById('cq-item-add')?.addEventListener('click', async () => {
      ...
      if (!name) { toast('名前 を 入れてください'); return; }
      try {
  ...
  document.querySelectorAll('[data-del-item]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (!confirm('この アイテム を 削除 します。 いいですか?')) return;
  ...
  return `
    <div class="row" style="padding:8px 10px; gap:8px; align-items:center; border:1.5px solid ${border}; border-radius:8px; background:${bg}; margin-bottom:6px">
      ...
      <div style="flex:1; min-width:0">
        ...
        <div class="hint-sm" style="font-size:10px; margin-top:2px">${it.total_visits} 人 が 制覇 ${it.added_by_name ? '・ 追加: ' + escapeHtml(it.added_by_name) : ''}</div>
      </div>
