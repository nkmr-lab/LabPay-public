<?php
// /api/cg2 — 自作ゲーム v2 framework (#cg2)。
// p5.js で描画、 sharedValues を自動同期する准リアルタイム multiplayer。
// 詳細設計 → docs/CUSTOM_GAMES_V2.md
...
function cg2_kinds_create(PDO $pdo, array $cfg): void {
    ...
    try {
        ...
        if (str_contains($e->getMessage(), 'Duplicate')) throw new ApiException('bad_request', 'slug が既に使われています', 400);
        throw $e;
    }
...
function cg2_kinds_update(PDO $pdo, array $cfg, string $slug): void {
    ...
    if ((int)$row['provider_user_id'] !== (int)$u['id'] && !$isAdmin) {
        throw new ApiException('forbidden', '提供者または admin のみ', 403);
    }
    $body = read_json_body();
    $sets = []; $args = [];
    ...
    if (array_key_exists('js_body', $body))     {
        ...
        if (strlen($js) > CG2_MAX_JS_BYTES) throw new ApiException('bad_request', 'JS が大きすぎます', 400);
...
function cg2_games_join(PDO $pdo, array $cfg, int $gid): void {
    ...
    db_tx($pdo, function () use ($pdo, $gid, $u, $uid) {
        ...
        if ($g['status'] !== 'waiting') throw new ApiException('bad_request', '募集中ではありません', 400);
        $k = cg2_kind_by_id($pdo, (int)$g['kind_id']);
        $players = json_decode($g['players_json'], true) ?: [];
        foreach ($players as $p) if ((int)$p['uid'] === $uid) throw new ApiException('bad_request', '既に参加済み', 400);
...
function cg2_games_add_ai(PDO $pdo, array $cfg, int $gid): void {
    ...
    db_tx($pdo, function () use ($pdo, $gid, $u) {
        ...
        $g = $st->fetch(PDO::FETCH_ASSOC);
        if (!$g) throw new ApiException('not_found', 'not found', 404);
        if ((int)$g['host_user_id'] !== (int)$u['id']) throw new ApiException('forbidden', '起案者のみ', 403);
        if ($g['status'] !== 'waiting') throw new ApiException('bad_request', '募集中ではありません', 400);
        $k = cg2_kind_by_id($pdo, (int)$g['kind_id']);
        $players = json_decode($g['players_json'], true) ?: [];
        // mixed 不可: 人間が 2 人以上いる場合 AI 追加 NG
        $humanCount = 0;
        foreach ($players as $p) if (empty($p['is_ai'])) $humanCount++;
        if ($humanCount >= 2) throw new ApiException('bad_request', '人間が 2 人以上いる卓に AI は入れられません', 400);
        if (count($players) >= (int)$k['max_players']) throw new ApiException('bad_request', '満員', 400);
        // bot user を 1 体拾う
        ...
        if (!$pickBot) throw new ApiException('bad_request', 'AI user の空きがありません', 400);
...
function cg2_games_start(PDO $pdo, array $cfg, int $gid): void {
    ...
    db_tx($pdo, function () use ($pdo, $cfg, $gid, $u) {
        ...
        if (count($players) < (int)$k['min_players']) {
            throw new ApiException('bad_request', sprintf('最低 %d 人で開始', (int)$k['min_players']), 400);
        }
        // 場代徴収 (人間のみ全員から)
        $fee = (int)$k['fee'];
        if ($fee > 0) {
            foreach ($players as $p) {
                ...
                if (Ledger::balanceOfUser($pdo, (int)$p['uid']) < $fee) {
                    throw new ApiException('insufficient_balance', "user#{$p['uid']} のポイント不足で開始不可", 400);
                }
            }
            foreach ($players as $p) {
                if (!empty($p['is_ai'])) continue;
...
function cg2_games_cancel(PDO $pdo, array $cfg, int $gid): void {
    ...
    db_tx($pdo, function () use ($pdo, $gid, $u) {
        ...
        if (!$g) throw new ApiException('not_found', 'not found', 404);
        if ((int)$g['host_user_id'] !== (int)$u['id']) throw new ApiException('forbidden', '起案者のみ', 403);
        if ($g['status'] !== 'waiting') throw new ApiException('bad_request', '募集中のみキャンセル可', 400);
...
function cg2_shared_post(PDO $pdo, array $cfg, int $gid): void {
    ...
    if (!is_array($changes) && !is_object($changes)) {
        // 全文投稿 (host.start で初期化時) も許可
        $changes = $body['values'] ?? null;
    }
    if ($changes === null) throw new ApiException('bad_request', 'changes / values が必要', 400);
    ...
    db_tx($pdo, function () use ($pdo, $gid, $u, $body, $changes) {
        ...
        if (!$isParticipant) throw new ApiException('forbidden', '参加者のみ', 403);
        if ($g['status'] !== 'playing' && $g['status'] !== 'waiting') {
            throw new ApiException('bad_request', 'playing/waiting のみ更新可', 400);
        }
    ...
    // 帰り値として新 seq + 全 values を返す (= 自分で merge 不要)
...
// ─── FINALIZE (= notifyResult が呼ばれたと host が報告) ────────
function cg2_games_finalize(PDO $pdo, array $cfg, int $gid): void {
    ...
    db_tx($pdo, function () use ($pdo, $cfg, $gid, $u, $text, $opts) {
        ...
        $k = cg2_kind_by_id($pdo, (int)$g['kind_id']);
        // 場代を提供者 90% / SYSTEM 10% に分配 (provider 未設定なら全額 SYSTEM)
        $players = json_decode($g['players_json'], true) ?: [];
        $humanCount = 0;
        foreach ($players as $p) if (empty($p['is_ai'])) $humanCount++;
        ...
        if ($totalFee > 0) {
            ...
            // SYSTEM (1) には既に全額入っている (cg2_games_start で transfer)
            // → provider がいれば SYSTEM → provider に share を払い直す
            if (!empty($k['provider_user_id']) && $providerCut > 0) {
                Ledger::transfer($pdo, 1, (int)$k['provider_user_id'], $providerCut, 'cg2_provider_share', 'cg2', $gid, "cg2 {$k['slug']} #{$gid} 場代提供者取り分");
            }
        }
...
function cg2_kind_by_slug(PDO $pdo, string $slug): array {
    ...
    $st->execute([$slug]);
    $r = $st->fetch(PDO::FETCH_ASSOC);
    if (!$r) throw new ApiException('not_found', 'kind がありません: ' . $slug, 404);
    return $r;
}
function cg2_kind_by_id(PDO $pdo, int $id): array {
    ...
    $st->execute([$id]);
    $r = $st->fetch(PDO::FETCH_ASSOC);
    if (!$r) throw new ApiException('not_found', 'kind がありません', 404);
    return $r;
}
function cg2_game_by_id(PDO $pdo, int $id): array {
    ...
    $st->execute([$id]);
    $r = $st->fetch(PDO::FETCH_ASSOC);
    if (!$r) throw new ApiException('not_found', 'game がありません', 404);
    return $r;
}
function cg2_validate_kind(array $body): array {
    $slug = trim((string)($body['slug'] ?? ''));
    if (!preg_match('/^[a-z0-9][a-z0-9-]{0,39}$/', $slug)) throw new ApiException('bad_request', 'slug は英小文字 / 数字 / - のみ (1-40)', 400);
